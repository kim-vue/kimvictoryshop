...
<table class="table">
  <tbody>
    ...
    <tr v-if="!isNew">
      ...
      <td v-if="value.fnlApprDtm">{{ this.$moment(value.fnlApprDtm).format('YYYY-MM-DD') }}</td>
      <td v-else></td>
    </tr>
    ...
    <tr>
      ...
      <th>정산구분</th>
      <td>
        <div v-if="value.stlFgCd === 'Y'">선급금정산</div>
        <div v-else>미정산</div>
      </td>
    ...
    <tr>
      <th>거래처</th>
      <td colspan="2">{{ value.evdCustCd }} / {{ value.evdCustNm }}</td>
    ...
    <tr>
      ...
      <th>사업장</th>
      <td colspan="2">{{ value.bpCd }} / {{ value.bpNm }}</td>
    </tr>
