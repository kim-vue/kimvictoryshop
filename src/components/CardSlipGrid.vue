<template>
    <div class="table-b">
        ...
        <div class="grid-wrap" ref="test">
            <div id="gridbox" class="grid-tbl-box grid-slip"></div>
        </div>
...
export default {
  ...
  methods: {
    ...
    drawGrid() {
      ...
      // iljinGrid.attachHeader("합계,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#stat_total,#stat_total,,,",["","text-align:right;","text-align:right;"]);
      // iljinGrid.setColAlign('center,center,center,center,center,center,center,center,center,center,right,right,right,center,center,center');
      ...
      iljinGrid.attachEvent("onCellChanged", (rId,cInd,nValue) => {
        ...
        if(cInd === 4 || cInd === 7) {
          ...
          const acctNm = this.$refs.grid.cells(rId,8).getValue();

          if(dcCd === 'D' && lnTypeCd === 'ITEM') {
            this.getRemainBudget(rId, cctrCd,acctCd);
            this.$emit("checkNdct", {acctCd, acctNm});
          }
        }
    ...
    setDefaultLine(expItemCnt) {
      this.addRow({ dcCd: 'C', lnTypeCd: 'ITEM', acctCd: this.line.creditAcctCd, acctNm: this.line.creditAcctNm, search: false });
    ...
    setBaseDept() {
      this.doLineAction(this.$refs.grid, {
        all: (grid,rId) => {
          grid.cells(rId,4).setValue(this.form.baseDeptCd);
          grid.cells(rId,5).setValue(this.form.baseDeptCd);
        }
      });
    },
    setHdSgtxt() {
      this.doLineAction(this.$refs.grid, {
        all: (grid,rId) => {
          grid.cells(rId,13).setValue(this.form.hdSgtxt);
        }
      });
    },
    setPurchAmt() {
      this.doLineAction(this.$refs.grid, {
        credit: (grid,rId) => {
          grid.cells(rId,12).setValue(this.toPure(this.form.totAmt)||0);
        }
      });
    },
    setApprAmt() {
      this.doLineAction(this.$refs.grid, {
        cost: (grid,rId) => {
          ...
          grid.cells(rId,11).setValue(amt);
        }
      });
    },
    setVatAmt() {
      this.doLineAction(this.$refs.grid, {
        vat: (grid,rId) => {
          grid.cells(rId,11).setValue(this.toPure(this.form.vatAmt)||0);
        },
      });
    },
    setCellValue(data) {
    ...
    validation() {
      ...
      for(var i=0; i <rowsNum; i++) {
        ...
        console.log(dcCd, lnTypeCd, debitAmt)
        ...
        if(!debitAcctCd) {
          ...
          return false;
          break;
        } else if(dcCd === 'D' && lnTypeCd === 'ITEM' && _.toInteger(debitAmt) == 0) {
